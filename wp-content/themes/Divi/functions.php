...
'woo_filter_upload_shippings'        );

function woo_filter_upload_shippings( $address_fields ) { 
//  $address_fields['file_upload']['required'] = true;
...
@ini_set( 'max_execution_time', '300' );
 function add_file_fields(){

 $uploadFile   = "";
 $uploadFile   .='<div class="success-div-response"></div><div id="upload_CNIC_image">';
 $uploadFile   .='<h3 style="margin-top: 30px; font-size:14px;">Upload your file here</h3>
 <div class="upload_img"><i class="fa fa-upload" aria-hidden="true"></i></div>';
 $uploadFile .='<input id="file_upload" name="file_upload" type="file"><input id="file_upload_sava" name="" type="button" value="Save File"><span class="Success-div"><img  src="http://www.consult.tax/wp-content/themes/Divi/images/loader.gif" width="100px"></span>';
 $uploadFile .='<span id="">';
 $uploadFile .='</span>';
 $uploadFile .='</div>';
 echo $uploadFile;
  echo "<style>.Success-div{display:none}</style>";
 }
 add_action('woocommerce_after_order_notes','add_file_fields');


add_action( 'wp_ajax_woocommerce_checkout_file_upload','woocommerce_checkout_file_upload' );
  add_action( 'wp_ajax_nopriv_woocommerce_checkout_file_upload','woocommerce_checkout_file_upload' );

 //session_start();
  function woocommerce_checkout_file_upload(){
    ...
    $time=strtotime(current_time( 'mysql' ));
      ...
      if ($movefile && !isset($movefile['error'])) {
           echo $_SESSION['attachment_file'] =$movefile['url'];
            //setcookie("file_url", $movefile['url'], time()+3600);
        } else {
        /**
...
function wpse_141088_upload_dir( $dir ) {
    return array(
        'path'   => $dir['basedir'] . '/file_uploaded',
        'url'    => $dir['baseurl'] . '/file_uploaded',
        'subdir' => '/file_uploaded',
    ) + $dir;
}
add_action('woocommerce_after_order_notes', 'customise_checkout_field');
function customise_checkout_field($checkout)
{
	if(isset($_COOKIE['file_url'])) {
		$cfileurl=$_COOKIE['file_url'];
	}
  woocommerce_form_field('file_upload_url', array(
    'type' => 'text',
    'class' => array(
      'my-field-class form-row-wide'
    ) ,
    'id' => __('file_upload_url') ,
    'name' => __('file_upload_url') ,
  ) , '');
 echo "<style>#file_upload_url{display:none}</style>";
}
//session_destroy();
...
//if (!$_POST['file_upload_url']) wc_add_notice(__('Please upload the file!') , 'error');

}
add_action('woocommerce_checkout_update_order_meta', 'customise_checkout_field_update_order_meta');
function customise_checkout_field_update_order_meta($order_id)
{
  if (!empty($_POST['file_upload_url'])) {
  	//setcookie( 'consulttax', sanitize_text_field($_POST['file_upload_url']), time() + (3600), "/" );
    update_post_meta($order_id, 'file_uploaded_url', sanitize_text_field($_POST['file_upload_url']));

  }
}
function cloudways_display_order_data_in_admin( $order ){  ?>
    
        <h3 style="margin-top:30px;float: left;"><?php _e( 'Additional Information', 'woocommerce' ); ?></h3>
        
        <?php $oder_checkout_image=get_post_meta( $order->id, 'file_uploaded_url', true );

         $file_name_final=explode("/",$oder_checkout_image);
         $html='<p class="form-field form-field-wide"><label>user uploaded file:</label>';
          if($file_name_final[6]!=""){ 
			 	$html.="<a href='".site_url()."/response-downloader?link=".$file_name_final[6]."' data-url='".$file_name_final[6]."' class='downloaded_link woocommerce-button button view'>Download </a>";
...
//             $new_columns['order-ship-to'] = __( 'File Name', 'textdomain' );
//              $new_columns['file-actions'] = __( 'Downloads', 'textdomain' );
//         }
//     }
...
function sv_wc_my_orders_ship_to_column( $order ) {
  ...
  //echo $order->id;
   $file_name=get_post_meta( $order->id, 'file_uploaded_url', true );
...
function sv_wc_my_file_actions_column( $order ) {

   $file_name=get_post_meta( $order->id, 'file_uploaded_url', true );
   $file_name_final=explode("/",$file_name);
   if($file_name_final[6]!=""){
echo "<a href='javascript:void(0);' data-url='".$file_name_final[6]."' class='downloaded_link woocommerce-button button view'>Download </a>";
}else{
  echo "";
}
 
}
add_action( 'woocommerce_my_account_my_orders_column_file-actions', 'sv_wc_my_file_actions_column' );

add_action( 'wp_head', 'my_getcookie' );
function my_getcookie() {
	 $cookie_name = "file_upload_url";
   $alert = isset( $_COOKIE[$cookie_name] ) ? $_COOKIE[$cookie_name] : 'not set';
   //echo "<script type='text/javascript'>alert('$alert')</script>";
}

add_action( 'woocommerce_view_order', 'view_order_and_thankyou_page', 2 );
...
if(isset($_POST['upload']))
{
	...
	if(!empty($_FILES['order_file_upload']['name'])) {
         
        // Setup the array of supported file types. In this case, it's just PDF.
        $supported_types = array('application/pdf');
        $files = $_FILES["order_file_upload"];
         add_filter( 'upload_dir', 'wpse_141088_upload_dir' );
      ...
      update_post_meta($order_id, 'order_attachment', $uploadedimages);  
      $page_link= admin_url()."post.php?post=".$order_id."&action=edit";
     $html='<!DOCTYPE html><html lang="en-US"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><title>Consultax</title></head><body leftmargin="0" marginwidth="0" topmargin="0" marginheight="0" offset="0"><div id="wrapper" dir="ltr"><table border="0" cellpadding="0" cellspacing="0" height="100%" width="100%"><tr><td align="center" valign="top"><div id="template_header_image"></div><table border="0" cellpadding="0" cellspacing="0" width="600" id="template_container"><tr><td><table border="0" cellpadding="0" cellspacing="0" width="600" id="m_695180551569644950template_header" style="background-color:#557da1;color:#ffffff;border-bottom:0;font-weight:bold;line-height:100%;vertical-align:middle;font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;border-radius:3px 3px 0 0"><tbody><tr><td id="m_695180551569644950header_wrapper" style="padding:36px 48px;display:block"><h1 style="font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;font-size:30px;font-weight:300;line-height:150%;margin:0;text-align:left;color:#ffffff">User has been uploaded files from his dashboard.</h1></td></tr></tbody></table></td></tr><tr><td align="center" valign="top"><table border="0" cellpadding="0" cellspacing="0" width="600" id="template_header"><tr><td id="header_wrapper"><h1>Order: #'.$order_id.'</h1></td></tr></table></td></tr><tr>
								<td align="center" valign="top"><table border="0" cellpadding="0" cellspacing="0" width="600" id="template_body"><tr><td valign="top" id="body_content"><table border="0" cellpadding="20" cellspacing="0" width="100%"><tr><td valign="top"><div id="body_content_inner"><p>Check Order when clicking on below link : </p><h2><a class="link" href="'.$page_link.'">[Order #'.$order_id.']</a></h2><div style="margin-bottom: 40px;"><table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: Helvetica Neue, Helvetica, Roboto, Arial, sans-serif;" border="1"><tbody></tbody><tfoot></tfoot></table></div>';

			//$to = "pawan@staginglabs.in";
			$to = get_option( 'admin_email' ); 

			$subject = '[Order#'.$order_id.'] User has Uplaoded Files' ;
			$body = $html;
			$headers = array('Content-Type: text/html; charset=UTF-8'); 
			wp_mail( $to, $subject, $body, $headers );
			 $url=site_url()."/mi-cuenta/view-order/".$order_id."/";
			wp_redirect( $url );
			exit;
   ...
   <section class="upload-files upload-files-by-admin">
  	<?php  $file_upload=get_post_meta( $order_id, 'file_uploaded_url', true );
   	if(!empty($file_upload)){
   		$file_name_final=explode("/",$file_upload);
   		 ?> 
   	 <h2>Upload file at the time of checkout</h2>
   	<div id="upload_CNIC_image_checkout">
    	...
    	<tbody>
   		...
   		if($file_name_final[6]!=""){
   			?>
				 	<tr><td>1</td>
				 		<td><?php echo $file_name_final[6]; ?></td>
						<td style='text-align: center;'>
						 <a href='javascript:void(0);' data-url="<?php echo $file_name_final[6]; ?> " class='downloaded_link woocommerce-button button view'><i class='fa fa-download' aria-hidden='true'> Download </a></td></tr>
...
<!--   <table class="woocommerce-table woocommerce-table--order-details shop_table order_details">
    	...
    	<tbody> -->
  <?php $orderimg=get_post_meta($order_id, 'order_attachment',true);
   // echo "<pre>";print_r($orderimg);die;
  $j=1;
  for($i=0;$i<count($orderimg);$i++){
            if(!empty($orderimg[$i]['url'])){
            	$oparts = explode('/', $orderimg[$i]['url']);
				$olast = array_pop($oparts);
				$oparts = array(implode('/', $oparts), $olast);
				 $file_name_final=explode("/",$orderimg[$i]['url']);
				 if($file_name_final[6]!=""){ 
				 	$html.="<tr><td>".$j."</td><td>".$oparts[1]."</td>";
									$html.="<td style='text-align: center;'><a href='javascript:void(0);' data-url='".$file_name_final[6]."' class='downloaded_link btn-download  button woocommerce-button button view'><i class='fa fa-download' aria-hidden='true'></i>Download </a><a data-order-id=".$order_id." href='javascript:void(0);' id=".$orderimg[$i]['file']."  class='delete_order_attachment btn-delete button'><i class='fa fa-trash' aria-hidden='true'></i>Delete</a></td></tr>";

				 	// $html.="<a href='javascript:void(0);' data-url='".$file_name_final[6]."' class='downloaded_link woocommerce-button button view'>Download </a>";
				 	// $html.='</br>';
				 }
...
<!--  <section class="upload-files upload-files-by-admin">
 	 ...
 	 <h2>Upload a new file</h2>
 	 	<div id="upload_CNIC_image">
  	<form name="form1" method="post" class="form1" enctype="multipart/form-data" style="margin-bottom: 15px;"> 
 <input id="order_file_upload" name="order_file_upload[]" type="file" size="10" multiple>
 <input type="hidden" name="orderid" value="<?php echo $order_id; ?>">
<span id="">

 </span>
 <input type="submit" name="upload" value="Upload" class="woocommerce-button button" id="order_file_uploadbtn">
 </form>
</div>
 </section> -->
 <?php echo "<style>.Success-div{display:none}.upload-files #upload_CNIC_image{
...
//add_action( 'woocommerce_view_order', 'view_admin_uploaded_documents', 1 );
 
function view_admin_uploaded_documents( $order_id ){   ?>
   <hr><br>
   ...
   <section class="upload-files upload-files-by-admin">
    ...
    <table class="woocommerce-table woocommerce-table--order-details shop_table order_details">
    	...
    	<tbody>
    		<?php  $orderimg=get_post_meta($order_id, 'wp_custom_attachment',true);
  				if(!empty($orderimg)){
  					$j=1;
  						for($i=0;$i<count($orderimg);$i++){
            					if(!empty($orderimg[$i]['url'])){
								...
								$oparts = array(implode('/', $oparts), $olast);
								 $file_name_final=explode("/",$orderimg[$i]['url']);
										if($file_name_final[6]!=""){ 
									$html.="<tr><td>".$j."</td><td>".$oparts[1]."</td>";
									$html.="<td style='text-align: center;'><a href='javascript:void(0);' data-url='".$file_name_final[6]."' class='downloaded_link btn-download button woocommerce-button button view'><i class='fa fa-download' aria-hidden='true'></i>Download </a></tr>";
...
function wp_order_attachment() { 
	//echo get_the_ID();
$pro_order=get_post_meta(get_the_ID(), 'order_attachment', true); 
 for($i=0;$i<count($pro_order);$i++){
            if(!empty($pro_order[$i]['url'])){
            	$oparts = explode('/', $pro_order[$i]['url']);
				$olast = array_pop($oparts);
				$oparts = array(implode('/', $oparts), $olast);
				 $file_name_final=explode("/",$pro_order[$i]['url']);
            	 if($file_name_final[6]!=""){ 
				 	$phtml.="<a href='".site_url()."/response-downloader?link=".$file_name_final[6]."' data-url='".$file_name_final[6]."' class='downloaded_link  view'>".$oparts[1]." </a>";
				 	$phtml.='</br>';
			 }

                // $phtml.='<a download target="_blank" href="'.$pro_order[$i]['url'].'"> '.$oparts[1].' </a>';
...
function wp_custom_attachment() {
    ...
    $html .= '</p>';
    $html .= '<input type="file" multiple id="wp_custom_attachment" name="wp_custom_attachment[]" value="" size="25" />';
    $img = get_post_meta(get_the_ID(), 'wp_custom_attachment', true);
    $html .='<hr>';
    $html.="<table cellpadding='0' cellspacing='0' class='woocommerce_order_items'><thead><tr><th class='item sortable' data-sort='string-ins'>File Name</th><th class='item_cost sortable' data-sort='float'>Download</th></tr></thead><tbody>";

    for($i=0;$i<count($img);$i++){
            if(!empty($img[$i]['url'])){
            	 $file_name_final=explode("/",$img[$i]['url']);
            	 if($file_name_final[6]!=""){ 
				 	$html.="<tr class='item'><td>".$file_name_final[6]."</td><td><a href='".site_url()."/response-downloader?link=".$file_name_final[6]."' data-url='".$file_name_final[6]."' class='downloaded_link woocommerce-button button view'>Download </a></td></tr>";
			 }


                // $html.='<a download target="_blank" href="'.$img[$i]['url'].'"> Download from Here </a>';
                // $html.='</br>';
              }
    }
    $html .='<hr>';
...
function save_custom_meta_data($id) {
    ...
    if(!empty($_FILES['wp_custom_attachment']['name'])) {
     ...
     //add_post_meta($id, 'wp_custom_attachment', json_encode($uploadedImages));
     update_post_meta($id, 'wp_custom_attachment', $uploadedImages);  
     $order = new WC_Order($id);
	$page_link= get_permalink( get_option('woocommerce_myaccount_page_id') );
     $html='<!DOCTYPE html>
<html lang="en-US">
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
		<title>Consultax</title>
	</head>
			...
			<table border="0" cellpadding="0" cellspacing="0" height="100%" width="100%">
				<tr>
					<td align="center" valign="top">
						...
						<tr>
						<td>
						<table border="0" cellpadding="0" cellspacing="0" width="600" id="m_695180551569644950template_header" style="background-color:#557da1;color:#ffffff;border-bottom:0;font-weight:bold;line-height:100%;vertical-align:middle;font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;border-radius:3px 3px 0 0"><tbody><tr>
						<td id="m_695180551569644950header_wrapper" style="padding:36px 48px;display:block">
								<h1 style="font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;font-size:30px;font-weight:300;line-height:150%;margin:0;text-align:left;color:#ffffff">Admin has been uploaded files.</h1>
											</td>
										</tr></tbody></table>
										</td>
										</tr>
...
<p>Below are the uploaded links url : </p>

<h2>
	<a class="link" href="'.$page_link.'view-order/'.$id.'/">[Order #'.$id.']</a></h2>
...
$subject = '[Order#'.$id.'] Admin Uplaoded Files Conformation Mail' ;
$body = $html;
$headers = array('Content-Type: text/html; charset=UTF-8');
...
function wdm_add_shipping_method_to_order_email( $order, $is_admin_email ) {
	...
	$file_name=get_post_meta( $order->id, 'file_uploaded_url', true );
	if(!empty($file_name)){
			echo '<p><h4>Uploaded file From user:</h4> <a download href="'.$file_name.'">Download </a></p><br>';	
	}    
}
...
$resp_html=' <div class="tabs">';
$i=1;
  foreach ($product_cat as $parent_product_cat)
  {

 $resp_html.='<div class="tab"><input type="checkbox" id="chck'.$i.'" name="rd">
        <label class="tab-label" for="chck'.$i.'">'.$parent_product_cat->name.'</label>';
  $child_args = array(
              'taxonomy' => 'product_cat',
              'hide_empty' => false,
  ...
  {
     $resp_html.='<div class="tab-content"><a href="'.get_term_link($child_product_cat->term_id).'">'.$child_product_cat->name.'</a></div>';
  }
 $resp_html.=" </div>";
   $i++; }
...
add_shortcode('woocommerce_taxonomys' , 'woocommerce_taxonomy');
...
// add_action( 'wp_ajax_downloaded_uploaded_file','downloaded_uploaded_file' );
// add_action( 'wp_ajax_nopriv_downloaded_uploaded_file','downloaded_uploaded_file' );
// function downloaded_uploaded_file(){


// }
 
  function downloaded_uploaded_file_code(){
		if(!empty($_REQUEST['link'])){
			$file="/var/www/wp-content/uploads/file_uploaded/".$_REQUEST['link'];
     	if (file_exists($file)) {
...
add_shortcode('downloaded_attchment' , 'downloaded_uploaded_file_code');
// add_action( 'woocommerce_before_cart', 'process_a');
// function process_a() { 
// 		echo '<ul class="breadcrumb-checkout">
// <li class="active"><a href="/"><span class="breadcrumb_text">Home > </span></a></li>
// <li><a href="/"><span class="breadcrumb_text">shop > </span></a></li>
// <li><a href="/"><span class="breadcrumb_text">Cart > </span></a></li></ul>';
//  }
// add_action( 'woocommerce_before_checkout_form', 'process_b');
// function process_b() {
//  echo '<ul class="breadcrumb-checkout">
// <li><a href=""><span class="breadcrumb_text">Home > </span></a></li>
// <li class="active"><a href="#"><span class="breadcrumb_text">shop > </span></a></li>
// <li class="active"><a href="#"><span class="breadcrumb_text">cart > </span></a></li>
// <li><a href="#"><span class="breadcrumb_text">checkout > </span></a></li></ul>'; 
// }
...
function search_product_func( $atts ){ 
$args = array(
          'taxonomy' => 'product_cat',
          'hide_empty' => false,
          'parent'   => 0
  ...
  $product_cat = get_terms( $args );
$resp_html=' <a id="open_tabs" type="button" value="">Show all Categoría</a> <div class="tabs sidebar-shop">';
$i=1;
$category_current = get_queried_object();
 ...
 foreach ($product_cat as $parent_product_cat){
 	...
 	if($category_current->parent==$parent_product_cat->term_id){
  		$class="active";
  	}else{
  		$class="";
  	}

 $resp_html.='<div class="tab '.$class.'"><input type="checkbox" id="chck'.$i.'" name="rd">
        <label class="tab-label" for="chck'.$i.'">'.$parent_product_cat->name.'</label>';
  $child_args = array(
              'taxonomy' => 'product_cat',
              'hide_empty' => false,
...
add_shortcode( 'search_product', 'search_product_func' );
function ship_to_different_address_translation( $translated_text, $text, $domain ) {
switch ( $translated_text ) {
case "Shipping address" :
$translated_text = __( "Mailing Address", "woocommerce" );
break;
...
function delete_order_attachment(){
	if(!empty($_POST['order_id'])){
		...
		$array_images=array();
		$orderimg=get_post_meta($order_id, 'order_attachment',true);
		for($i=0;$i<count($orderimg);$i++){
			
			if($orderimg[$i]['file']==$_POST['data_url']){
					unlink($orderimg[$i]['url']);
			}else{
				echo 2;
				$array_images[]=$orderimg[$i];
			}
		}

		update_post_meta($order_id,'order_attachment',$array_images);
...
function remove_shipping_phone_field($fields) {
    $fields ['shipping_first_name']['required'] = false; // To be sure "NOT required"
